...
$Q$ and displays $R$, but it does not define, reinterpret, or extend OPS-1 semantics.

OPS-2 variation is admissible \emph{only} within the closed extension space above.
No interface layer may weaken invariants or gates, nor may it introduce control
semantics, feedback loops, learning, heuristic shortcuts, or STOP bypass.

\paragraph{Auto-disqualification by semantic contamination.}
If a toolkit layer (or its documentation/positioning) implies that the Engine Core
is a ``tool'', ``method'', ``framework'', ``best practice'', ``playbook'', or
