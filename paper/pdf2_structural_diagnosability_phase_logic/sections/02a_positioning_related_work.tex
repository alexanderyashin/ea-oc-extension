...
Despite overlapping terminology—such as state, structure, observation, and performance—these traditions are typically oriented toward objectives that differ fundamentally from the problem considered here.

This paper focuses on \emph{structural diagnosability} under incomplete observability: whether the available observation surface permits a principled distinction between structurally incompatible enterprise states, or whether such a distinction is formally impossible.
The contribution is diagnostic rather than prescriptive.
Negative outcomes—non-identifiability, undecidability, or STOP conditions—are treated as informative epistemic results rather than as failures to be remedied.
...
The enterprise setting addressed here differs in a structurally relevant way.
...
The focus thus shifts from reconstruction to diagnostic decidability.
...
The present work adopts the diagnostic stance of explicit hypothesis spaces and admissibility conditions, but without assuming that a complete fault taxonomy or a uniquely identifiable structure exists.
...
Instead, it isolates a necessary precondition for rational intervention: that the enterprise state is diagnostically well-posed under available evidence.
...
In practice, EA resolves this through process, consensus, and social validation.
...
Both can support action under uncertainty, but neither directly addresses the epistemic question of whether a structural diagnosis is warranted.

Indicators and narratives may remain actionable even when the underlying structure is not uniquely identifiable.
