...
\label{subsec:decidable-predicates}

\begin{proposition}[Observationally invariant predicates]
\label{prop:observational-invariance}
...
predicates: those whose truth value depends only on admissible observable
signatures. No claim is made that such predicates are structurally rich,
informative, or diagnostically meaningful; only their logical
decidability in the sense of Definition~\ref{sec:definitions} is
asserted.

\begin{theorem}[Decidability of STOP boundary predicates]
\label{thm:stop-decidability}
...
internally admissible states compatible with a given observable
signature. Under the stated invariance condition, STOP becomes a class
property of observational equivalence.

\subsection{Undecidable predicates}
\label{subsec:undecidable-predicates}

\begin{theorem}[Structural undecidability]
\label{thm:structural-undecidability}
...
\item internal configuration within $\Omega_{EA}$ away from
      $\partial\Omega_{EA}$;
\item relative ordering, distance, or margin of thresholds
      $\Theta_{EA}$ not observable through $\Pi$;
...
\end{itemize}
are undecidable in the sense of
Definition~\ref{subsec:decidability}.

\begin{corollary}[Undecidability of non-terminal phase distinctions]
\label{cor:nonterminal-undecidability}
...
observations, even in the absence of noise or data limitations.

\subsection{STOP-only decidability}
\label{subsec:stop-only-decidability}
...
If STOP membership is observationally invariant, while the remaining
phase values are not, then $\varphi$ is STOP-only decidable.
\end{theorem}
...
\label{prop:exhaustive-trichotomy}
