...
Observability is defined via the admissible observation constructs
already present in ETS. Let
\[
...
\Sigma_{\mathrm{obs}}
\]
denote the space of observable signatures induced by admissible
observations. The observation operator $\Pi$ maps internal states to
observable signatures:
\[
\Pi : \Omega_{EA} \to \Sigma_{\mathrm{obs}}.
\]
No assumptions are made about injectivity, surjectivity, continuity, or
invertibility of $\Pi$ beyond what is explicitly specified in ETS.

\subsection{Observational equivalence}

\begin{definition}[Observational equivalence]
Two enterprise continua $K_{EA}^{(1)}$ and $K_{EA}^{(2)}$ are said to be
\emph{observationally equivalent} if their observable signatures
coincide:
\[
...
enterprise continua. All elements within such a class are
indistinguishable by any admissible observation, even if their internal
structures differ in axes, potentials, flows, cycles, or thresholds.
...
admissible OC/ETS elements. Typical examples include predicates
corresponding to admissible phase outcomes (OK, INERTIA, COLLAPSE,
STOP), satisfaction or violation of threshold conditions, or membership
in specific regions of $\Omega_{EA}$ or $\partial\Omega_{EA}$.

Formally, a state predicate is a mapping
\[
\varphi : \Omega_{EA} \to \{\text{true},\text{false}\},
\]
with $\varphi$ defined solely using OC/ETS constructs and relations.
...
$K_{EA}^{(1)}, K_{EA}^{(2)}$ that are observationally equivalent, the
predicate agrees:
\[
\Pi\!\left(\Omega_{EA}^{(1)}\right)
=
...
Decidability is thus a purely logical property: it requires invariance
of the predicate across all members of an observational equivalence
class. No algorithmic, statistical, inferential, or operational
...
continua $K_{EA}^{(1)}, K_{EA}^{(2)}$ such that
\[
\varphi\!\left(\Omega_{EA}^{(1)}\right)
\neq
\varphi\!\left(\Omega_{EA}^{(2)}\right).
...
Undecidability expresses the impossibility of determining the truth
value of $\varphi$ from any admissible observation, regardless of data
volume, observational persistence, or analytical sophistication.
...
non-terminal states remain logically undecidable under incomplete
observability.
...
invariance of predicates under observational equivalence). The
remainder of the paper relies on this separation as a structural
constraint, not as a methodological choice.
