...
and its executable specification ETS.K\_EA~\cite{occore,etsk_ea}.
...
Admissibility is determined prior to any diagnostic procedure or
...
An admissible observation \(O\) is \emph{compatible} with a model
realization \(s \in \Omega(K_{EA})\) if all statements in \(O\) can be
embedded into \(s\) without violating any mandatory invariant, threshold,
